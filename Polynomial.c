...
void mul_poly(struct node **,struct node **,struct node **);
...
void mul_poly(struct node **st1,struct node **st2,struct node **st3){
    struct node *t1,*t2;
    for(t1=*st1;t1!=NULL;t1=t1->next){
        for(t2=*st2;t2!=NULL;t2=t2->next){
            creat_poly(st3,t1->coff*t2->coff,t1->exp+t2->exp);
        }
    }
}